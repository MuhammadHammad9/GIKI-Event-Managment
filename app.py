...
def booking():
    ...
    if request.method == 'POST':
        ...
        # Redirect to the additional_booking page with event ID and tickets as URL parameters
        return redirect(url_for('additional_booking', event_id=event_id, tickets=tickets))
    else:
        events = fetch_events()
        return render_template('booking.html', events=events)

@app.route('/additional_booking', methods=['POST', 'GET'])
def additional_booking():
    if request.method == 'POST':
        # Correctly retrieve data from POST request body
        event_id = request.form.get('event')
        tickets = request.form.get('tickets')
        return render_template('additional_booking.html', event_id=event_id, tickets=tickets)
    else:
        return redirect(url_for('booking_page'))  # redirect to booking page on GET
